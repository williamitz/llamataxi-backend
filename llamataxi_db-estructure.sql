...
) ENGINE = InnoDB AUTO_INCREMENT = 6385 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Compact;
...
CREATE TABLE `ts_journal_driver`  (
  ...
  `totalJournal` float(10, 2) NULL DEFAULT 0.00,
...
CREATE TABLE `ts_service`  (
  ...
  `discount` float(5, 2) NULL DEFAULT 0.00,
  `discountType` char(6) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT 'NONEDD',
  PRIMARY KEY (`pkService`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 833 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Compact;
...
delimiter ;;
CREATE PROCEDURE `as_sp_getListVehicleDriver`(IN `InPage` INT, IN `InPkDriver` INT, IN `InPkBrand` TINYINT, IN `InPkModel` TINYINT, IN `InIsProper` TINYINT, IN `InNumberPlate` VARCHAR(15), IN `InYear` TINYINT, IN `InColor` VARCHAR(20), IN `InStatus` TINYINT)
BEGIN
...
BEGIN
	#Routine body goes here...
	DECLARE outError, vErrFound, vErrExpired, vErrStatus, vErrRole TINYINT DEFAULT 0;
	...
	SELECT IF( pkCoupon IS NULL , 1, 0),
				 IF( dateExpiration <= CURRENT_DATE() , 2, 0),
				 IF( vRole != roleCoupon , 4, 0 ),
				 IF( statusRegister = 0 , 8, 0)
	
	INTO vErrFound, vErrExpired, vErrRole, vErrStatus
	FROM rb_coupon
	WHERE codeCoupon = InCode;
	
	SET outError = vErrFound + vErrExpired + vErrRole + vErrStatus;
...
BEGIN
	...
	SET outFields =  "  C.pkCoupon,
											...
											C.statusRegister,
											C.dateRegister,
											IF( C.dateExpiration <= CURRENT_DATE() , 1, 0) AS 'isExpired' ";
...
BEGIN
	#Routine body goes here...
	DECLARE outError, vInPkDriver INT DEFAULT 0;
	...
	SELECT IF(pkVehicle IS NULL, 64, 0),
					IF( as_vehicle.verified = 0 , 128, 0)
	INTO vFoundVehicle, vVerifiedVeh
	FROM as_vehicle
	WHERE pkVehicle = InFkVehicle AND fkDriver = vInPkDriver;
	...
	SET outError = vFoundService + vErrService + vErrStatus + vErrVerified + vErrStatusDriver + vFoundDriver + vFoundVehicle + vVerifiedVeh + vFoundOffer;
...
BEGIN
	...
	SET outError = ts_fn_verify_addOfferService( InPkService, InPkOffer, InFkDriver, InFkVehicle );
...
BEGIN
	...
	IF InCategory = 2 THEN
		SET InWhere = CONCAT(InWhere, " AND S.fkCategory BETWEEN 2 AND 3 ");
	ELSEIF InCategory = 3 THEN
		SET InWhere = CONCAT(InWhere, " AND S.fkCategory = 3 ");
	-- ELSE
		-- SET InWhere = CONCAT(" WHERE S.statusService = 1 AND S.statusRegister = 1 AND S.indexHex = '",InIndexUser,"' ");
	END IF;
...
BEGIN
	...
	IF vPkCategory = 2 THEN
		SET InWhere = CONCAT(InWhere, " AND fkCategory BETWEEN 2 AND 3 ");
	ELSEIF vPkCategory = 3 THEN
		SET InWhere = CONCAT(InWhere, " AND fkCategory = 3 ");
	ELSE
		SET InWhere = " WHERE statusRegister = 1 AND statusService = 1";
	END IF;
